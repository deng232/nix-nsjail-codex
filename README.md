...
- The current working directory is mounted read-write and used as the jailed
  working directory.
- Parent environment variables are not inherited wholesale. Only variables
  named in `config/env.allowlist` are copied when present.
...
- `$BROWSER` is set to `browserchannel` inside the jail, and
  `$BROWSERCHANNEL_FIFO` is generated by the wrapper.
